...
#==================================== FUNCTIONS =====================================
#....................................................................................
def binarization(image, size = (480,240), thresHold=150):
    img_gray = cv.cvtColor(src = image, code = cv.COLOR_BGR2GRAY)
    img_gauss = cv.GaussianBlur(src = img_gray, ksize =  (5,5), sigmaX = 0)
    thr, img_thr = cv.threshold( img_gauss, thresHold, 255, cv.THRESH_BINARY)
    img_r = cv.resize( src = img_thr, dsize = size, interpolation = cv.INTER_NEAREST)
    return (img_r)
#....................................................................................
def poly_interest(image, pt = [ (135,150), (350, 150), (380, 238), (115, 238) ], colour = (100,100,100), closed = True):
    polygon_vert = np.array(pt)
    polygon_vert = polygon_vert.reshape((-1,1,2))
    return cv.polylines(image, [polygon_vert], closed, colour)
#....................................................................................
...
def right_sum( image, midpoint ):
    return np.round(np.sum( image[:, midpoint:].sum(axis = 0) )/(255*image.shape[0]*image.shape[1]),2)
#==================================== PARAMETERS =====================================
#defining new vertices
points = [ (105 - 70,150), (355 + 70, 150), (85 - 70, 238), (390 + 70, 238) ]
points_pline = points.copy()
points_pline[2], points_pline[3] = points_pline[3], points_pline[2]
...
thickness = 1
#==================================== CODE =====================================
video = cv.VideoCapture('data/video_isaacarroyov.mp4')
while(video.isOpened()):
    ret, frame = video.read()
    if ret:
        cv.imshow("Original video", frame)
        img_bin = binarization(frame, thresHold=90)
        img_bin = poly_interest(image=img_bin, pt = points_pline)
        cv.imshow("Binarized video", img_bin)
        img_interest = region_interest(image=img_bin, pt = points)
